...
public class UserController {
	...
	public String editMovie(@PathVariable Long id, HttpSession session, RedirectAttributes attributes,
			ModelMap model) {
		return userService.editMovie(id, session, attributes, model);
	}

	@PostMapping("/update-movie")
	public String updateMovie(@Valid MovieDto movieDto, BindingResult result, RedirectAttributes attributes,
			HttpSession session, @RequestParam Long id, ModelMap model) {
		return userService.updateMovie(movieDto, result, attributes, session, id, model);
	}
	...
	public String loadBookings(ModelMap model, RedirectAttributes attributes, HttpSession session) {
	    return userService.loadBookings(model, attributes, session);
	}
